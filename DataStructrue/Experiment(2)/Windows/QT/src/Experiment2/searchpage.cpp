...
searchPage::searchPage(QWidget *parent) : QWidget(parent)
{
    ...
    avlsearch=new QPushButton(QString::fromLocal8Bit("AVL树搜索"));
    hashsearch=new QPushButton(QString::fromLocal8Bit("哈希表搜索"));
	info=new QLabel();
...
void searchPage::searchButtonClicked(bool useAVL)
{
    ...
    info->setText(QString::fromLocal8Bit("搜索总耗时")+QString::number(end-start)+QString::fromLocal8Bit("毫秒; ")+QString::fromLocal8Bit("共搜索得到")+QString::number(result.size())+QString::fromLocal8Bit("条结果;"));
}
