...
invertedDocs::invertedDocs(vector<pageInfo> &pinfos, wordSegmentation *_dic)
{
	dic=_dic;
	clock_t start,end;
	cout<<"Start building the AVL tree"<<endl;
	start=clock();
	mtree = new mAVL<CharString,docLink>();
	bool taller=false;
	bool success=false;
	...
	cout<<"Finish building the AVL tree"<<endl;
	cout<<"Building the AVL tree spent: "<<(end-start)<<" milliseconds\n"<<endl;

	cout<<"Start building the Hash table"<<endl;
	start=clock();
	mhash=new mHash(pinfos);
	end=clock();
	cout<<"Finish building the Hash table"<<endl;
	cout<<"Building the Hash table spent: "<<(end-start)<<" milliseconds\n"<<endl;
}
...
docLink* invertedDocs::Search(CharString key)
{
	...
	cout<<"AVL tree"<<" query spent: "<<(end-start)<<" milliseconds"<<endl;
	if(reavl)
...
docLink* invertedDocs::hashSearch(CharString key)
{
	...
	cout<<"Hash tree"<<" query spent: "<<(end-start)<<" milliseconds"<<endl;
	return re;
}
...
vector<pair<int,int> > invertedDocs::getResult(vector<CharString> queries,bool choose)
{
	//string retext="AVL tree";
	//if(choose==false)
	//	retext="Hash table";
	//clock_t start,end;
	//start=clock();
	...
	//end=clock();
	//cout<<retext<<" query spent: "<<(end-start)<<" milliseconds"<<endl;
	return re;
}
...
void invertedDocs::batchSearch(string inputfilename,string outputfilename,bool choose)
{
	...
	while(getline(fin,line))
	{
		...
		vector<pair<int,int> > re=getResult(queries,choose);
		printResult(re,fout);
	}
}
