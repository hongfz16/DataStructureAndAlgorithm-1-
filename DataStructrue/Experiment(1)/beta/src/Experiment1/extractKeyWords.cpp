...
void print_info(CharString* info,int _size,ofstream& os,wordSegmentation& dic,UnicodeToGBK& u2g)
{
	for(int i=0;i<_size-1;++i)
	{
		if(i==3 && !info[3].empty())
		{
			...
			if(info[3].indexOf(sub,0)==-1)
			{
				...
				info[3]=replacecomma;
				CharString newinfo3;
				for(int i=0;i<info[3].getSize();++i)
				{
					if(i<info[3].getSize() && info[3].getIndex(i)>0 && info[3].getIndex(i)<=127)
					{
						newinfo3.append((char)0);
						newinfo3.append(info[3].getIndex(i));
						++i;
						while(i<info[3].getSize() && info[3].getIndex(i)>0 && info[3].getIndex(i)<=127)
						{
							newinfo3.append(info[3].getIndex(i));
							++i;
						}
			...
			for(int ii=0;ii<size;++ii)
			{
				...
				if(flag==false)
				{
					/*
					wstring wwstring;
					wwstring+=((wchar_t)sum);
					string sstring;
					WStringToString(wwstring,sstring);
					os<<sstring;
					*/
	...
	//cout<<"here"<<endl;
	/*
	ofstream cacheos("cache");
	cacheos<<info[2];
	cacheos.close();
	ifstream cacheis("cache");
	string cachetemp;
	CharString segcs;
	while(!cacheis.eof())
	{
		cachetemp.clear();
		cacheis>>cachetemp;
		segcs.append(cachetemp);
	}
	*/
	//cout<<segcs.is_equal(info[2])<<endl;
	//cout<<"here again"<<endl;
	CharString newinfo2;
	for(int i=0;i<info[2].getSize();++i)
	{
		if(i<info[2].getSize() && info[2].getIndex(i)>0 && info[2].getIndex(i)<=127)
		{
			newinfo2.append((char)0);
			newinfo2.append(info[2].getIndex(i));
			++i;
			while(i<info[2].getSize() && info[2].getIndex(i)>0 && info[2].getIndex(i)<=127)
			{
				newinfo2.append(info[2].getIndex(i));
				++i;
			}
